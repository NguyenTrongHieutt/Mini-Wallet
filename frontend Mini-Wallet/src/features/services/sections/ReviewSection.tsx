...
export function ReviewSection({ service }: { service: Service; readOnly: boolean }) {
  ...
  const results = useQueries({ queries: [
    { queryKey: ['serviceFields', service.id], queryFn: () => serviceApi.listFields(service.id) },
    { queryKey: ['serviceValidations', service.id], queryFn: () => serviceApi.listValidations(service.id) },
    { queryKey: ['serviceDefinition', service.id], queryFn: async () => { try { return await serviceApi.definition(service.id) } catch (error) { if (error instanceof ApiError && error.code === 404) return { transDefinition: null }; throw error } } },
  ] })
  ...
  const publish = useMutation({ mutationFn: () => serviceApi.publish(service.id), onSuccess: ({ service: next }) => client.setQueryData(['service', service.id], { service: next }) })
  const unpublish = useMutation({ mutationFn: () => serviceApi.unpublish(service.id), onSuccess: ({ service: next }) => client.setQueryData(['service', service.id], { service: next }) })
